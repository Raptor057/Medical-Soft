...
namespace Medical.Office.Domain.Repository
{
    public interface IPatientsData
    {
        ...
        Task<IEnumerable<MedicalAppointmentReminderCalendarHostedService>> GetMedicalAppointmentRemindersCalendarListAsync();

        #endregion
        
        Task<IEnumerable<PatientAdvancement>> GetPatientAdvancementByIDPatientAsync(long IDPatient);
        
        Task<PatientAdvancement> GetPatientAdvancementByIDAsync(long ID);
        Task InsertPatientAdvancementAsync(long IDPatient, string Concept, decimal? Quantity);
        Task UpdatePatientAdvancementAsync(string Concept, decimal? Quantity, bool Active, long Id);
